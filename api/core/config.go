...
func NewDefaultConfig() *types.AppConfig {
	return &types.AppConfig{
		Listen:   "0.0.0.0:5678",
		MysqlDns: "root:12345678@tcp(localhost:3306)/geekai_agent?charset=utf8mb4&collation=utf8mb4_unicode_ci&parseTime=True&loc=Local",
		ProxyURL: "",
		...
		TikaHost:    "http://localhost:9998",
		GeekApiHost: "https://sapi.geekai.me",
		AdminSession: types.Session{
			SecretKey: utils.RandString(64),
			MaxAge:    86400,
		},
	}
...
func LoadConfig(configFile string) (*types.AppConfig, error) {
	...
	if config.GeekApiHost == "" {
		config.GeekApiHost = "https://sapi.geekai.me"
	}
	applyEnvOverrides(config)
...
func applyEnvOverrides(config *types.AppConfig) {
	if value := os.Getenv("GEEKAI_MYSQL_DSN"); value != "" {
		config.MysqlDns = value
	}
	if value := os.Getenv("GEEKAI_REDIS_PASSWORD"); value != "" {
		config.Redis.Password = value
	}
	if value := os.Getenv("GEEKAI_SESSION_SECRET"); value != "" {
		config.Session.SecretKey = value
	}
...
func LoadSystemConfig(db *gorm.DB) *types.SystemConfig {
	...
	sysConfig.Id = 0
	db.Where("name", "license").First(&sysConfig)
	...
	// 加载 Coze API 配置
	var cozeConfig types.CozeApiConfig
	sysConfig.Id = 0
	db.Where("name", "coze").First(&sysConfig)
	err = utils.JsonDecode(sysConfig.Value, &cozeConfig)
	if err != nil {
		logger.Error("load coze config error: ", err)
	}

	// 加载 GeekService 配置
	var captchaConfig types.CaptchaConfig
	sysConfig.Id = 0
	db.Where("name", "captcha").First(&sysConfig)
	err = utils.JsonDecode(sysConfig.Value, &captchaConfig)
	if err != nil {
		logger.Error("load geek service config error: ", err)
	}
	if captchaConfig.ApiURL == "" {
		captchaConfig.ApiURL = "https://sapi.geekai.me"
	}
	// 加载短信配置
	var smsConfig types.SMSConfig
	sysConfig.Id = 0
	db.Where("name", "sms").First(&sysConfig)
	...
	sysConfig.Id = 0
	db.Where("name", "oss").First(&sysConfig)
	...
	sysConfig.Id = 0
	db.Where("name", "smtp").First(&sysConfig)
	...
	sysConfig.Id = 0
	db.Where("name", "payment").First(&sysConfig)
	...
	sysConfig.Id = 0
	db.Where("name", "wx_login").First(&sysConfig)
	err = utils.JsonDecode(sysConfig.Value, &wxLoginConfig)
	if err != nil {
		logger.Error("load wechat login config error: ", err)
	}
