...
func (h *RedeemHandler) RegisterRoutes() {
	rg := h.App.Engine.Group("/api/admin/redeem/")
...
func (h *RedeemHandler) Export(c *gin.Context) {
	...
	for _, item := range items {
		records = append(records, []string{item.Name, item.Code, fmt.Sprintf("%d", item.Amount), item.CreatedAt.Format("2006-01-02 15:04:05")})
	}
...
// BatchRemove 批量删除兑换码
func (h *RedeemHandler) BatchRemove(c *gin.Context) {
	var data struct {
		Ids []int `json:"ids"`
	}
	if err := c.ShouldBindJSON(&data); err != nil {
	...
	err := h.DB.Where("id IN ?", data.Ids).Delete(&model.Redeem{}).Error
	if err != nil {
		resp.ERROR(c, err.Error())
		return
	}
