...
func NewFileHandler(app *core.AppServer, db *gorm.DB, manager *oss.UploaderManager) *FileHandler {
	return &FileHandler{BaseHandler: BaseHandler{App: app, DB: db}, uploaderManager: manager}
}

// RegisterRoutes 注册路由
func (h *FileHandler) RegisterRoutes() {
	rg := h.App.Engine.Group("/api/file/")
