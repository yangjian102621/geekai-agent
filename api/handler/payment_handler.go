...
// SyncOrders 同步订单状态
func (h *PaymentHandler) SyncOrders() {
	defer func() {
	...
	if err != nil {
		logger.Errorf("error with query orders: %v", err)
		return
	}
	...
	for _, order := range orders {
		...
		case payment.PayChannelGeek:
			res, err = h.geekPayService.Query(order.OrderNo)
			logger.Debugf("聚合支付订单查询结果：%+v", res)
			if err != nil {
...
func (h *PaymentHandler) Pay(c *gin.Context) {
	...
	case "wxpay":
		...
		// 优先使用微信官方支付
		if h.config.WxPayConfig.Enabled {
			data.Channel = "wxpay"
			if h.config.WxPayConfig.Domain != "" {
				data.Domain = h.config.WxPayConfig.Domain
			}
		...
		} else if h.config.EPayConfig.Enabled { // 聚合支付
			logger.Debugf("聚合支付%+v", data)
			data.Channel = payment.PayChannelGeek
			if h.config.EPayConfig.Domain != "" {
				data.Domain = h.config.EPayConfig.Domain
			}
			...
			r, err := h.geekPayService.Pay(params)
	...
	case "alipay":
		if h.config.AlipayConfig.Enabled {
			logger.Debugf("支付宝，%+v", data)
			data.Channel = payment.PayChannelAL
			if h.config.AlipayConfig.Domain != "" { // 用于本地调试支付
				data.Domain = h.config.AlipayConfig.Domain
			}
...
func (h *PaymentHandler) paySuccess(info payment.OrderInfo) error {
	...
	// 增加用户算力
	err = h.userService.IncreaseScores(int(order.UserId), remark.Credit, model.ScoreLog{
		Type:    types.ScoreRecharge,
		Subject: remark.Subject,
		Remark:  fmt.Sprintf("充值积分，金额：%f，订单号：%s", order.Amount, order.OrderNo),
	})
	if err != nil {
		return err
	}
	...
	// 更新产品销量
	err = h.DB.Model(&model.Product{}).Where("id = ?", order.Pid).
...
func (h *PaymentHandler) GeekPayNotify(c *gin.Context) {
	...
	// 查询订单状态
	order, err := h.geekPayService.Query(params["out_trade_no"])
	if err != nil {
		logger.Error(err)
		c.String(http.StatusOK, "fail")
		return
