...
func NewSmtpService(sysConfig *types.SystemConfig) *SmtpService {
	return &SmtpService{
...
func (s *SmtpService) SendVerifyCode(to string, code int) error {
	subject := fmt.Sprintf("%s 验证码", s.config.AppName)
	body := fmt.Sprintf("【%s】：您的验证码为 %d，请不要告诉他人。如非本人操作，请忽略此邮件。", s.config.AppName, code)
	logger.Debugf("smtp config: %+v", utils.JsonEncode(s.config))
